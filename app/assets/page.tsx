  ...
  return (
    <div className="px-5 pt-4 pb-6 space-y-5">
      ...
      <p className="text-[12px] text-ink-muted text-center pt-2 leading-relaxed">
        잔액은 시작 잔액 + 모든 거래 합산으로 계산돼요. 대출은 순자산에서 자동으로 차감됩니다
      </p>
    </div>
  );
}
      ...
      <div className="flex items-baseline justify-between px-1 mb-3">
        ...
        <span
          className={`text-[16px] font-medium tabular ${
            group.total < 0 ? "text-coral-800" : "text-ink"
          }`}
        >
            ...
            <li key={a.id}>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-[16px] font-medium tabular ${
                      displayBalance < 0 ? "text-coral-800" : "text-ink"
                    }`}
                  >
                    <KoreanAmount value={displayBalance} precision="exact" />
                  </span>
                </div>
                <div className="text-[12px] text-ink-muted mt-1">
                  ...
                  {a.balance !== a.initial_balance && (
                    <>
                      {" · 시작 "}
                      <KoreanAmount value={displayInit} precision="man" />
                    </>
                  )}
                </div>
              </Link>
